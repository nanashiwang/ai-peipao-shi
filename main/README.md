...
POST /api/agent/replies/auto-draft
...
AI 回复页提供「自动生成待审回复」按钮：它会按当前账号可访问家庭批量生成 `ai_reply` 草稿，写入 `ai_outputs.status=needs_review`，跳过无有效家长消息或近 24 小时已有待审回复的家庭；该按钮不创建发送任务，也不会触发企业微信发送。

家庭画像 Agent 会同步维护家长档案字段：关注点、沟通风格、满意度评级、风险信号、续报意向、学生状态和建议动作。家庭详情、AI 画像页和陪跑会话侧栏会优先展示这些结构化字段。

家庭详情支持人工记录跟进动作，覆盖电话、私信、群提醒、周报、补课、投诉和续报沟通。记录会写入 `followup_records`，同时进入家庭时间线、工作台待办和今日优先级，避免低分反馈或投诉升级被遗漏。
...
- 家长账号登录后进入「家长看板」，只读展示本人家庭的课程阶段、打卡/PBL 进度、已审核周报和最近沟通，可对周报点击签收并提交满意度反馈；低分反馈会回流跟进记录，不返回内部风控、审核和运营排障数据。
...
项目支持新增微信客服 API 通道：家长在个人微信里咨询，服务端接收消息后复用现有 `AI回复 Agent` 的 Prompt、知识库和安全规则，并通过微信客服 API 回复。该通道不需要 Windows 被控端；原企业微信好友/群聊仍可继续走会话存档 + RPA。

微信客服通道强制校验 48 小时回复窗口、单轮最多 5 条、文本 2048 字节、客户/客服账号绑定和高风险人工复核。配置、回调、客户绑定与上线检查见 `docs/微信客服接入.md`。

## 企业微信客户联系

项目支持把“家长添加企业成员”接入为长期客户关系：关系回调自动建档，后台定时同步客户和负责成员，新客户可自动发送欢迎语，并可按计划创建一对一企业群发任务。企业群发接口成功只代表任务创建完成，仍需对应成员在企业微信客户端确认，确认前客户不会收到。

客户联系不提供聊天正文。需要 AI 读取双方私聊内容时仍需开通会话内容存档；系统会按 `external_userid + 成员 userid` 自动归并到同一家庭档案。普通私聊的即时直接发送仍走现有 RPA，客户联系适合欢迎语、任务安排和通知。完整配置见 `docs/客户联系接入.md`。
